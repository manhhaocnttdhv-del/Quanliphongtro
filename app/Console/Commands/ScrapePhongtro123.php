...
class ScrapePhongtro123 extends Command
{
        ...
        foreach ($citiesToRun as $city) {
            ...
            for ($page = 1; $page <= $pages; $page++) {
                ...
                foreach ($listings as $listing) {
                    $detail = $this->parseDetailPage($listing['url']);
    ...
    private function parseDetailPage(string $url): array
    {
        ...
        if (preg_match('/Địa chỉ[:\s]*([^<\n]{10,200})/ui', $html, $m)) {
            $detail['address'] = trim(strip_tags(html_entity_decode($m[1])));
        }
    ...
    private function importRoom(array $listing, array $detail, User $landlord, array $city): ?Room
    {
        ...
        if (Room::where('name', $name)->exists()) return null;
    ...
    private function extractAmenities(string $text): array
    {
        $amenities = [];
        $map = [
            'wifi'       => 'Wifi',        'wc riêng'   => 'WC riêng',
            'khép kín'   => 'Khép kín',    'nước nóng'  => 'Nước nóng',
            'nóng lạnh'  => 'Nước nóng',   'máy lạnh'   => 'Máy lạnh',
            'điều hòa'   => 'Điều hòa',    'điều hoà'   => 'Điều hòa',
            'tủ lạnh'    => 'Tủ lạnh',     'máy giặt'   => 'Máy giặt',
            'gác lửng'   => 'Gác lửng',    'gác xép'    => 'Gác xép',
            'ban công'   => 'Ban công',    'bếp'        => 'Bếp nấu',
            'camera'     => 'Camera',      'để xe'      => 'Chỗ để xe',
            'giữ xe'     => 'Chỗ để xe',   'tự do'      => 'Tự do giờ giấc',
            'nội thất'   => 'Nội thất',    'chung chủ'  => 'Không chung chủ',
        ];
